...
(function () {
  ...
  var css = "" +
    ...
    "#onebox-root .lede{text-align:center;margin:0 auto;max-width:none;font-family:var(--headline);font-weight:700;font-size:clamp(16px,5vw,30px);line-height:1.25;letter-spacing:-.01em;color:#000}" +
    "#onebox-root .ledeoffer{color:var(--teal)}" +
    "#onebox-root h1.page{font-family:var(--headline);font-weight:600;font-size:clamp(12px,3.4vw,17px);line-height:1.3;text-align:center;margin:20px auto 0;max-width:none;color:#000}" +
  ...
  root.innerHTML =
    ...
    '<p class="lede">' + (C.congrats ? esc(C.congrats)
      : "Get " + (OFFERR ? '<b class="ledeoffer">' + esc(OFFERR) + "</b><br>" : "") + "All Permanent Makeup Packages!") + "</p>" +
  ...
  /* The congrats headline must never exceed 2 lines, no matter how long
     a client's offer custom value is or how narrow the phone. CSS gets
     it right for the template copy; this shrinks the font for outliers. */
  (function fitLede() {
    ...
    function fit() {
      ...
      while (fs > 12 && el.getBoundingClientRect().height / (fs * 1.25) > 2.1) {
        fs -= 1;
