...
export function buildFanbasisBlock(productId: string, redirectUrl: string): string {
  ...
  return [
    ...
    '<div style="font-size:13px;color:#34568a;line-height:1.5">\uD83D\uDD12 <b>Secure, encrypted checkout.</b></div>',
    ...
    // release added just hang when tapped inside the funnel (payments
    // stopped across every one-box client). Grant the token, then reload
    // the iframe once (same session URL) so it takes effect; the prefill
    // param rides along in the same single reload.
    "var pp=hasPrefill?encodeURIComponent(JSON.stringify(prefillObj)):'',a=0,iv=setInterval(function(){a++;var f=document.querySelector('#fanbasis-checkout-wrapper iframe');if(f&&f.src){try{f.sandbox.add('allow-payment-request');}catch(e){}var src=f.src;if(pp&&src.indexOf('prefill=')===-1)src=src+'&prefill='+pp;f.src=src===f.src?src+'&obr=1':src;clearInterval(iv);}if(a>20)clearInterval(iv);},200);",
