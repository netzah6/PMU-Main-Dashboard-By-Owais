...
import type { V3Client } from "@/lib/ppa";

// One row per PPS client: fee, deposits, what has been charged, what is ready
// to charge, and the profitability columns. Extracted from /api/ppa/overview so
// the coach view (/api/ppa/coach) reports the SAME numbers from the SAME code —
// a coach and an admin looking at "Ready" must never see different money.
//
// All counts are DEPOSIT-LINKED and time-aware: each deposit is resolved to its
// lead's stage AND scheduled appointment, so "ready to charge" = appointments
// that actually happened (served or past-due) and aren't charged yet.
...
export async function buildPpaClients(roster: V3Client[]) {
...
export type PpaClientRow = Awaited<ReturnType<typeof buildPpaClients>>[number];
