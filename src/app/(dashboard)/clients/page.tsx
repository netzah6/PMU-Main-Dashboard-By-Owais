...
export default function ClientsPage() {
  ...
  // Program per client for the list rows (PPS vs Standard) — shared with the
  // profile badge and the Reports tab so the three can never disagree.
  const clientProgram = useCallback(
    (c: ClientRecord) => programOf(lookupPayment(payments, c.owner_name)),
    [payments]);

  const selectedPayment = useMemo(
    () => (selectedClient ? lookupPayment(payments, selectedClient.owner_name) : null),
